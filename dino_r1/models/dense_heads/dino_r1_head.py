...
class DINO_R1_Head(DINOHead):
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
        ...
        Q = grpo_meta['select_score_per_q'].shape[1]
        all_layer_cls_score = outs[0][:, :, -Q:, :]
        all_layer_bbox_pred = outs[1][:, :, -Q:, :]
        grpo_loss = self.grpo_loss(
            per_token_score=grpo_meta['select_score_per_q'], 
            select_id_per_q=grpo_meta['select_id_per_q'], 
            ref_select_score_per_token=grpo_meta['ref_select_score_per_token'], 
            old_select_score_per_token=grpo_meta['old_select_score_per_token'], 
            all_layer_cls_score=all_layer_cls_score, 
            all_layer_bbox_pred=all_layer_bbox_pred, 
            img_metas=batch_img_metas, 
            batch_gt_instances=batch_gt_instances, 
            grpo_advantage_weight=grpo_meta['grpo_alpha'],
            grpo_beta=grpo_meta['grpo_beta'],
        )

        Ndec = len(grpo_loss)
        for ndi in range(Ndec):
            losses['grpo_loss_{}'.format(ndi)] = grpo_loss[ndi]
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,
        ...
        text_masks = self.text_masks.new_zeros(
            (self.text_masks.size(0), cls_out_channels))
    ...
    def _get_dn_targets_single(self, gt_instances: InstanceData,
        ...
        Returns:
            ...
            - neg_inds (Tensor): Sampled negative indices for each image.
        """
        cls_out_channels = gt_instances.positive_maps.shape[-1]
    ...
    def grpo_loss(self,
                  per_token_score, 
                  select_id_per_q, 
                  ref_select_score_per_token, 
                  old_select_score_per_token, 
                  all_layer_cls_score, 
                  all_layer_bbox_pred, 
                  img_metas, 
                  batch_gt_instances, 
                  grpo_advantage_weight,
                  grpo_beta = 0.04,
                  grpo_num_mute = 300,
                  grpo_epsilon_low = 0.2,
                  grpo_epsilon_high = 0.28,
                  epsilon_avoid_nan = 2e-9, 
                  ):
                
        B = len(batch_gt_instances)
        Ndec = all_layer_cls_score.shape[0]
        all_per_token_advantage = [] 
        for ndi in range(Ndec):
            per_token_advantage = [] 
            for bi in range(B):
                cls_score = all_layer_cls_score[ndi, bi]
                bbox_pred = all_layer_bbox_pred[ndi, bi]
                img_h, img_w = img_metas[bi]['img_shape']
                ...
                pred_instances = InstanceData(scores=cls_score, bboxes=bbox_pred)
                # assigner and sampler
                advantage = self.assigner.get_grpo_advantage(
                    pred_instances=pred_instances,
                    gt_instances=batch_gt_instances[bi],
                    img_meta=img_metas[bi]) 
                per_token_advantage.append(advantage) 
            per_token_advantage = torch.stack(per_token_advantage, dim=0) 
            all_per_token_advantage.append(per_token_advantage)
        
        ref_per_token_score = torch.gather(ref_select_score_per_token, 1, select_id_per_q) 
        ref_per_token_score = ref_per_token_score.sigmoid() 
        per_token_score = per_token_score.sigmoid() 

        # Compute the KL divergence between the model and the reference model
        per_token_kl = (ref_per_token_score + epsilon_avoid_nan) / (per_token_score + epsilon_avoid_nan) - torch.log(ref_per_token_score + epsilon_avoid_nan) + torch.log(per_token_score + epsilon_avoid_nan) - 1 

        # Compute the loss
        all_losses = []
        for per_token_advantage in all_per_token_advantage:
            per_token_advantage = (per_token_advantage - per_token_advantage.mean(1, keepdim=True)) / (per_token_advantage.std(1, keepdim=True) + epsilon_avoid_nan) 
            per_token_advantage[:, -grpo_num_mute:] *= 0.0
            per_token_loss = -per_token_advantage * grpo_advantage_weight + grpo_beta * per_token_kl 
